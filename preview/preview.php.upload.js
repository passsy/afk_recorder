  ...
  uploadToPHPServer(file, function(progress, videoURL) {
    ...
    if (progress != "Upload started...") {
      ...
      browserCache.innerHTML =
        ...
        progress +
        ' style="margin-top: 10px;"></progress>';
    } else {
    ...
    if (progress >= 99 || videoURL || progress === "progress-ended") {
      browserCache.innerHTML = "";
      fname.innerHTML =
        "Uploaded to Server. Retrieving the private video URL...";
    }
  });
};
  ...
  request.onreadystatechange = function() {
    if (request.readyState == 4 && request.status == 200) {
      if (
        ...
        request.responseText.toString().indexOf("<h2>Upload failed.</h2>") === 0
      ) {
      ...
      if (
        request.responseText &&
        request.responseText.toString().indexOf("deleted successfully") !== -1
      ) {
        callback("deleted");
        return;
      }
      ...
      if (
        request.responseText &&
        request.responseText.toString() === "Problem deleting file."
      ) {
