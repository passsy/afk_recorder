...
function onGettingFile(f, item) {
  ...
  if (!file) {
    if (item && item.name) {
      header.querySelector("p").innerHTML =
        item.display + " has no video data.";
  ...
  var html =
    'This file is in your browser cache. Click <a href="' +
    file.url +
    '" download="' +
    file.name +
    '">here</a> to download.';
  if (item.php && item.youtube) {
    html =
      'Click to download file from <a href="' +
      item.php +
      '" target="_blank">Private Server</a> <img src="images/cross-icon.png" class="cross-icon" title="Delete from server"> or <a href="' +
      item.youtube +
    ...
    html =
      ...
      '" target="_blank">' +
      item.php +
      '</a> <img src="images/cross-icon.png" class="cross-icon" title="Delete from server">';
...
var btnRecordingsListDropDown = document.querySelector(
  ...
  if (btnRecordingsListDropDown.className === "visible") {
    ...
    DiskStorage.GetFilesList(function(list) {
      ...
      list.forEach(function(item) {
        ...
        div.innerHTML =
          '<img src="images/cross-icon.png" class="cross-icon"><img src="images/edit-icon.png" class="edit-icon">' +
          item.display;
        ...
        div.querySelector(".cross-icon").onclick = function(e) {
          ...
          if (
            !window.confirm(
              "Are you sure you want to permanently delete the selected recording?"
            )
          ) {
            return;
          }
        ...
        div.querySelector(".edit-icon").onclick = function(e) {
          ...
          var newFileName =
            prompt("Please enter new file name", item.display) || item.display;
